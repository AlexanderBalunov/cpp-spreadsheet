...
public:
    ...
    double Evaluate(const std::function<double(Position)>& args) const override {
        double result = 0;
        switch (type_) {
            case Add:       result = lhs_->Evaluate(args) + rhs_->Evaluate(args); break;
            case Subtract:  result = lhs_->Evaluate(args) - rhs_->Evaluate(args); break;
            case Multiply:  result = lhs_->Evaluate(args) * rhs_->Evaluate(args); break;
            case Divide:    result = lhs_->Evaluate(args) / rhs_->Evaluate(args); break;
        }
        // если значение result соответстует +inf, -inf или NaN - выбрасываем исключение
